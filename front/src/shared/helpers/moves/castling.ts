...
export const getCastlingMoves = ({ position, castleDirection, piece, number, letter }: Moves) => {
	...
	const kingStartColumn = isWhite ? 4 : 3
	...
	if (letter !== kingStartColumn || number % 7 !== 0 || castleDirection === 'none') {
		return moves
	}

	const row = number
	const rookLeft = isWhite ? 'wr' : 'br'
	const rookRight = isWhite ? 'wr' : 'br'
	...
	if (
		...
		!position[row][2] &&
		!position[row][1] &&
		position[row][0] === rookLeft &&
		!isPlayerInCheck({
			positionAfterMove: performMove({ position, piece, number, letter, x: row, y: 3 }),
			position,
			player
		}) &&
		!isPlayerInCheck({
			positionAfterMove: performMove({ position, piece, number, letter, x: row, y: 2 }),
			position,
			player
		})
	) {
	...
	if (
		...
		!position[row][5] &&
		!position[row][6] &&
		position[row][7] === rookRight &&
		!isPlayerInCheck({
			positionAfterMove: performMove({ position, piece, number, letter, x: row, y: 5 }),
			position,
			player
		}) &&
		!isPlayerInCheck({
			positionAfterMove: performMove({ position, piece, number, letter, x: row, y: 6 }),
			position,
			player
		})
	) {
